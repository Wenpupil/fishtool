...
public class RulerView extends View {
    ...
    private void setAttr(AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs, com.lsp.R.styleable.RulerView, defStyleAttr, 0);

        scaleLimit =  a.getInt(com.lsp.R.styleable.RulerView_scaleLimit, scaleLimit);

        rulerHeight = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_rulerHeight, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, rulerHeight, getResources().getDisplayMetrics()));

        rulerToResultgap = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_rulerToResultgap, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, rulerToResultgap, getResources().getDisplayMetrics()));

        scaleCount = a.getInt(com.lsp.R.styleable.RulerView_scaleCount, scaleCount);

        scaleGap = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_scaleGap, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, scaleGap, getResources().getDisplayMetrics()));

        minScale = a.getInt(com.lsp.R.styleable.RulerView_minScale, minScale)/scaleLimit;

        firstScale = a.getFloat(com.lsp.R.styleable.RulerView_firstScale, firstScale)/scaleLimit;

        maxScale = a.getInt(com.lsp.R.styleable.RulerView_maxScale, maxScale)/scaleLimit;

        bgColor = a.getColor(com.lsp.R.styleable.RulerView_bgColor, bgColor);

        smallScaleColor = a.getColor(com.lsp.R.styleable.RulerView_smallScaleColor, smallScaleColor);

        midScaleColor = a.getColor(com.lsp.R.styleable.RulerView_midScaleColor, midScaleColor);

        largeScaleColor = a.getColor(com.lsp.R.styleable.RulerView_largeScaleColor, largeScaleColor);

        scaleNumColor = a.getColor(com.lsp.R.styleable.RulerView_scaleNumColor, scaleNumColor);

        resultNumColor = a.getColor(com.lsp.R.styleable.RulerView_resultNumColor, resultNumColor);

        unitColor = a.getColor(com.lsp.R.styleable.RulerView_unitColor, unitColor);

        String tempUnit = unit;
        unit = a.getString(com.lsp.R.styleable.RulerView_unit);
        if (TextUtils.isEmpty(unit)) {
            unit = tempUnit;
        }

        smallScaleStroke = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_smallScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, smallScaleStroke, getResources().getDisplayMetrics()));

        midScaleStroke = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_midScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, midScaleStroke, getResources().getDisplayMetrics()));
        largeScaleStroke = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_largeScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, largeScaleStroke, getResources().getDisplayMetrics()));
        resultNumTextSize = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_resultNumTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, resultNumTextSize, getResources().getDisplayMetrics()));

        scaleNumTextSize = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_scaleNumTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, scaleNumTextSize, getResources().getDisplayMetrics()));

        unitTextSize = a.getDimensionPixelSize(com.lsp.R.styleable.RulerView_unitTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, unitTextSize, getResources().getDisplayMetrics()));

        showScaleResult = a.getBoolean(com.lsp.R.styleable.RulerView_showScaleResult, showScaleResult);
        isBgRoundRect = a.getBoolean(com.lsp.R.styleable.RulerView_isBgRoundRect, isBgRoundRect);
    ...
    private void drawScaleAndNum(Canvas canvas) {
                ...
                valueAnimator.addListener(new AnimatorListenerAdapter() {       //增加一个监听，用来返回给使用者滑动结束后的最终结果刻度值
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (onChooseResulterListener != null) {
                            onChooseResulterListener.onEndResult(year,resultText);
                        }
                    }
                });
        ...
        //这里是滑动时候不断回调给使用者的结果值
        currentScale = new WeakReference<>(new BigDecimal(((width / 2 - moveX) / (scaleGap * scaleCount)+minScale)*scaleLimit)).get().setScale(3, BigDecimal.ROUND_HALF_UP).floatValue() ;
        ...
        else result = (float)(first + last * 0.01);
        //resultText = String.valueOf(currentScale);
        //Wenpupil
        resultText = String.valueOf(result);
        int index = resultText.indexOf('.');
        year = resultText.substring(0,index);
        month = resultText.substring(index+1);
        //temp = temp + "年" + resultText.substring(index + 1);
        if(last - 10 == 0) month = month + "0";
        resultText = month;

        if (onChooseResulterListener != null) {
            onChooseResulterListener.onScrollResult(year,month); //接口不断回调给使用者结果值
        }
    ...
    public interface OnChooseResulterListener {
        void onEndResult(String year, String month);
        void onScrollResult(String year, String month);
    }
